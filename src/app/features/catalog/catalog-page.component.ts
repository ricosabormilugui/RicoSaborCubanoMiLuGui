...
@Component({
  ...
  template: `
    ...
    <section class="products-wrap" [attr.aria-busy]="isInitialLoading()">
      ...
      <div class="empty-state" *ngIf="!isInitialLoading() && !filteredProducts().length">No se encontraron productos.</div>
  ...
  styles: [
    ...
    `.filters input,.filters select{min-width:min(100%,260px);box-shadow:0 6px 16px var(--shadow-soft)}`,
    ...
    `.product{background:linear-gradient(180deg,color-mix(in srgb,var(--surface-1) 38%,var(--surface-0) 62%),color-mix(in srgb,var(--surface-0) 92%,var(--bg-elevated) 8%));border:1px solid color-mix(in srgb,var(--border-soft) 72%,transparent);border-radius:18px;overflow:hidden;display:flex;flex-direction:column;min-height:100%;box-shadow:0 8px 18px var(--shadow-soft);transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease,background .18s ease}`,
...
export class CatalogPageComponent {
  ...
  private updateSeo(): void {
    ...
    const path = category ? `/categoria/${encodeURIComponent(category)}` : '/';
    const title = category
      ? `${categoryLabel} de comida cubana`
      : 'Comida cubana a domicilio y recogida';
    const description = category
      ...
      : SEO_SITE_CONFIG.defaultDescription;
  ...
  private updateCategoryParam(value: string): void {
    void this.router.navigate(value ? ['/categoria', value] : ['/'], {
