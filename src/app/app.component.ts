...
@Component({
  ...
  `,
  styles: [
    `.top-banner{background:var(--accent-red);text-align:center;padding:6px;font-size:14px;color:#fff;font-weight:700}`,
    `.app-shell{width:100%;min-height:100vh;display:flex;flex-direction:column}`,
    `.navbar{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;width:100%;backdrop-filter:blur(10px);background:color-mix(in srgb, var(--surface-0) 88%, #000 12%);position:sticky;top:0;z-index:50;border-bottom:1px solid color-mix(in srgb, var(--border-soft) 80%, transparent)}`,
    ...
    `.badge{position:absolute;top:-5px;right:-5px;background:var(--accent-red);border-radius:50%;font-size:10px;padding:2px 5px;color:#fff}`,
