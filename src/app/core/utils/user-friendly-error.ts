...
export function getUserFriendlyError(error: unknown, fallback = DEFAULT_ERROR): string {
  ...
  if (source.kind === 'timeout') return 'No hemos podido conectar en este momento. Inténtalo de nuevo.';
  if (source.status === 401) return 'Tu sesión ha caducado. Vuelve a iniciar sesión.';
  ...
  if (source.status !== undefined && source.status >= 500) return fallback;
  if (source.status === 0 || source.kind === 'network') return 'No hemos podido conectar en este momento. Inténtalo de nuevo.';
