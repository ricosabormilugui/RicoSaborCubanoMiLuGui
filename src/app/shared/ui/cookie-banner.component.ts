...
@Component({
  ...
  template: `
    <section class="cookie-banner" *ngIf="!cookies.hasDecision()" role="dialog" aria-modal="true" aria-live="polite" aria-labelledby="cookie-banner-title">
      <div class="cookie-card">
        <div>
          <p class="eyebrow">Privacidad</p>
          <h2 id="cookie-banner-title">Configura tus cookies</h2>
        ...
        <div class="cookie-actions">
          ...
          <button class="btn" type="button" (click)="configuring.set(!configuring())" [attr.aria-expanded]="configuring()">Configurar</button>
  ...
  `,
  styles: [
    `.cookie-banner{position:fixed;inset:0;z-index:200;display:grid;align-items:end;max-height:100dvh;overflow-y:auto;overscroll-behavior:contain;padding:1rem 1rem max(1rem,env(safe-area-inset-bottom));background:linear-gradient(180deg,transparent 35%,var(--overlay-bg))}`,
    `.cookie-card{width:min(980px,100%);max-height:calc(100dvh - 2rem);margin:0 auto;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1rem;align-items:center;overflow-y:auto;border:1px solid var(--border-soft);border-radius:18px;padding:1rem;background:var(--surface-0);box-shadow:var(--shadow-card);color:var(--text-main)}`,
