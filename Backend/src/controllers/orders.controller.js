...
export async function createOrder(req, res) {
  try {
    ...
    const order = {
      ...
      status: "nuevo",
      statusHistory: [{ status: "nuevo", at: new Date().toISOString(), note: null, signature: null }]
    };
...
export async function listOrdersForAdmin(req, res) {
  try {
    ...
    const limit = req.query.limit ? Number(req.query.limit) : 100;
    const orders = await listOrders({ status, limit: Number.isFinite(limit) ? Math.min(limit, 500) : 100 });
    return res.status(200).json({ orders });
  } catch (error) {
    return res.status(500).json({ error: error.message ?? "Unexpected error" });
  }
...
export async function updateOrderStatusForAdmin(req, res) {
  try {
    ...
    const updated = await updateOrderStatus(orderId, status, {
      ...
      deliverySignature: status === "entregado" ? String(deliverySignature).trim() : null,
