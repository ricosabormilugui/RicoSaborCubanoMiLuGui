...
for(const outcome of ['success','401'])test(`restauración de sesión tardía (${outcome}) no restaura ni cierra otra cuenta`,async()=>{
  ...
  const {CustomerAuthService}=compile('src/app/core/services/customer-auth.service.ts',name=>{
    if(name==='@angular/core')return {...angular,inject:()=>({warning:()=>{throw new Error('stale warning');},dismissAll(){},close(){},adoptGuestCart:()=>true,adoptGuestShipping:()=>true,bindSession(){},syncAuthenticatedFavorites:async()=>true,refreshAvailability:async()=>{},products:()=>[],hasLiveCatalog:()=>false,syncInventory(){}})};
    if(name.endsWith('api.config'))return {resolveApiBaseUrl:()=>'/api'};
