...
test('la escena auth usa productos locales configurables y un success acotado', () => {
  const { AUTH_FLOATING_PRODUCTS, AUTH_VISUAL_SUCCESS_MS, AUTH_VISUAL_SUCCESS_REDUCED_MS, AUTH_VISUAL_LOGO_PX } = load('src/app/features/auth/auth-visual.model.ts');

  assert.equal(AUTH_VISUAL_SUCCESS_MS, 1550);
  assert.equal(AUTH_VISUAL_SUCCESS_REDUCED_MS, 400);
  assert.equal(AUTH_VISUAL_LOGO_PX, 186);
  assert.ok(AUTH_VISUAL_SUCCESS_MS >= 1500 && AUTH_VISUAL_SUCCESS_MS <= 1600);
...
test('el panel visual desktop monta la escena y no carga fotos CMS', () => {
  ...
  assert.match(html, /\*ngIf="!showVisual\(\)"/);
  assert.match(html, /class="auth-mark"/);
  assert.match(html, /auth-mark-spin--outer/);
  assert.match(html, /auth-mark-spin--middle/);
  assert.match(html, /auth-mark-spin--inner/);
  assert.doesNotMatch(html, /assets\/auth\/products\//);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /vortex-mark\.svg/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /-72vw/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /44s linear infinite/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /33s linear infinite/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /25s linear infinite/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /-webkit-mask-image/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /transform-origin: 50% 49\.5%/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /prefers-reduced-motion: reduce/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /min-width: 600px\) and \(max-width: 1023px/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /max-width: 599px/);
  assert.match(read('src/app/features/auth/auth-layout.component.css'), /margin-block:\s*auto/);
  assert.match(read('src/app/features/auth/auth-form.css'), /max-width: 599px/);
  assert.match(read('src/app/features/auth/auth-form.css'), /border-radius:\s*12px/);
  assert.match(read('src/app/features/auth/auth-form.css'), /font-weight:\s*600/);
  assert.match(read('src/app/features/auth/auth-form.css'), /height:\s*45px/);
  assert.match(read('src/app/features/auth/auth-form.css'), /color-mix\(in srgb, var\(--accent-red\)/);
  assert.doesNotMatch(read('src/app/features/auth/auth-form.css'), /content:\s*['"]→['"]/);
  assert.doesNotMatch(read('src/app/features/auth/auth-form.css'), /\.auth-submit[\s\S]*999px/);
  assert.match(read('src/app/app.component.ts'), /max-width:599px/);
  assert.match(read('src/app/app.component.ts'), /100svh/);
  assert.match(read('src/app/app.component.ts'), /flex:0 0 auto/);
  assert.match(read('src/app/app.component.ts'), /\[class\.is-auth\]="isAuthSurface\(\)"/);
  assert.match(read('src/app/app.component.ts'), /min-width:1024px\)\{\.page-content\.is-auth\{display:flex;flex-direction:column;min-height:0;overflow:hidden\}/);
  assert.doesNotMatch(read('src/app/features/auth/auth-layout.component.css'), /@media \(min-width: 1024px\)[\s\S]*margin-block:\s*auto/);
  assert.ok(existsSync(resolve(root, 'public/assets/auth/vortex-mark.svg')));
  const mark = read('public/assets/auth/vortex-mark.svg');
  assert.match(mark, /viewBox="0 0 800 800"/);
  assert.match(mark, /#005c94|#00[0-9a-f]{4}/i);
  assert.match(mark, /bdad85|#db2e42/i);
  assert.match(mark, /<circle /);
  assert.doesNotMatch(read('src/app/features/auth/auth-layout.component.ts'), /from ['"]three['"]/);
  assert.doesNotMatch(layout, /import\('\.\/auth-particle-vortex\.renderer'\)/);
  assert.match(html, /<app-auth-visual-stage/);
  assert.doesNotMatch(html, /auth-brand/);
  assert.doesNotMatch(read('src/app/features/auth/auth-layout.component.css'), /\.auth-brand/);
  ...
  assert.match(read('src/app/features/auth/auth-visual-stage.component.css'), /620ms/);
  assert.match(read('src/app/features/auth/auth-visual-stage.component.css'), /950ms/);
  assert.match(read('src/app/features/auth/auth-visual-stage.component.css'), /186px/);
  assert.match(read('src/app/features/auth/auth-visual-stage.component.css'), /mask-image/);
  assert.doesNotMatch(read('src/app/features/auth/auth-visual-stage.component.css'), /mix-blend-mode:\s*multiply/);
