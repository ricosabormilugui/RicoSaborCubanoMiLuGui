...
test('add-to-cart y páginas críticas refrescan disponibilidad', () => {
  ...
  assert.match(addToCart, /evaluateLiveAddToCart\(live, minimumQuantity\(live\), deps\.cart\.items\(\)\)/);
  assert.match(detail, /maxAddableQuantity/);
  assert.match(detail, /evaluateLiveAddToCart\(product, quantity, this\.cart\.items\(\)\)/);
  assert.match(cart, /pruneMissing/);
  assert.match(checkout, /pruneMissing/);
  assert.match(read('src/app/app.component.ts'), /reconcileCartInventory/);
  assert.match(read('src/app/core/services/customer-auth.service.ts'), /reconcileCartInventory/);
